...
class NN(nn.Module):
    def __init__(self, n_input, n_output, n_h, Num_Layer):
        ...
        self.inlayer = nn.Linear(2 * n_input, n_h).to(device)
        for _ in range(self.Num_Layer):
            #fc = nn.Linear(n_h,n_h)          
            #setattr(self,'fc%i'%i,fc)
            self.hencoding.append(nn.Linear(n_h, n_h).to(device))
        self.K = nn.Linear(n_h, n_h)
        for _ in range(self.Num_Layer):
            #fc = nn.Linear(n_h,n_h)          
            #setattr(self,'fc%i'%i,fc)
            self.hdecoding.append(nn.Linear(n_h, n_h).to(device))
        self.outlayer = nn.Linear(n_h, n_output).to(device)
...
class meta_surrogate_a(nn.Module):
    def __init__(self,params):
        ...
        self.a_Layer = NN(params['alayer'][0],params['alayer'][1],params['alayer'][2],params['alayer'][3]).to(device)#7, 1, 20, 5)
        self.Wslot_Layer = NN(params['wlayer'][0],params['wlayer'][1],params['wlayer'][2],params['wlayer'][3]).to(device)#2, 1, 20, 5)
    ...
    def forward(self, h1, z1, h2, z2, aold, qlast, dt, yFull, aFull):
        ...
        Wslot2 = self.Wslot_Layer(torch.cat([y2.float().view(-1,1),
                                             yFull.float().view(-1,1)],axis=1)).to(device)
        ...
        amid_tem = self.a_Layer(torch.cat([ymid.float().view(-1,1),
                                       ...
                                       dt.float().view(-1,1)],axis=1)).to(device)
        
        amid = torch.clip(amid_tem, torch.tensor(np.zeros(aFull.shape)).float().view(-1,1).to(device), aFull.float().view(-1,1)).to(device)
        return amid
...
class meta_surrogate_q(nn.Module):
    ...
    def __init__(self,param):
        ...
        # 根据SWMM的dynwave计算过程设计计算图
        self.Wslot_Layer = NN(param['wlayer'][0],param['wlayer'][1],param['wlayer'][2],param['wlayer'][3]).to(device)
        self.r_Layer = NN(param['rlayer'][0],param['rlayer'][1],param['rlayer'][2],param['rlayer'][3]).to(device)
        self.a_Layer = NN(param['alayer'][0],param['alayer'][1],param['alayer'][2],param['alayer'][3]).to(device)
        self.v_Layer = NN(param['vlayer'][0],param['vlayer'][1],param['vlayer'][2],param['vlayer'][3]).to(device)

        self.dq1_Layer = NN(param['dq1layer'][0],param['dq1layer'][1],param['dq1layer'][2],param['dq1layer'][3]).to(device)
        self.dq2_Layer = NN(param['dq2layer'][0],param['dq2layer'][1],param['dq2layer'][2],param['dq2layer'][3]).to(device)
        self.dq3_Layer = NN(param['dq3layer'][0],param['dq3layer'][1],param['dq3layer'][2],param['dq3layer'][3]).to(device)
        self.dq4_Layer = NN(param['dq4layer'][0],param['dq4layer'][1],param['dq4layer'][2],param['dq4layer'][3]).to(device)
        self.dq5_Layer = NN(param['dq5layer'][0],param['dq5layer'][1],param['dq5layer'][2],param['dq5layer'][3]).to(device)
        self.dq6_Layer = NN(param['dq6layer'][0],param['dq6layer'][1],param['dq6layer'][2],param['dq6layer'][3]).to(device)
        
        self.q_Layer = NN(param['qlayer'][0],param['qlayer'][1],param['qlayer'][2],param['qlayer'][3]).to(device)
    ...
    def forward(self, h1, z1, h2, z2, length, sigma, aold, amid, qlast, dt, yFull, aFull, rFull):
        ...
        Wslot1 = self.Wslot_Layer(torch.cat([y1.float().view(-1,1),yFull.float().view(-1,1)],axis=1)).to(device)
        Wslot2 = self.Wslot_Layer(torch.cat([y2.float().view(-1,1),yFull.float().view(-1,1)],axis=1)).to(device)

        a1 = self.a_Layer(torch.cat([y1.float().view(-1,1),
                                     Wslot1.float().view(-1,1),
                                     yFull.float().view(-1,1),
                                     aFull.float().view(-1,1)],axis=1)).to(device)
        ...
        r1 = self.r_Layer(torch.cat([y1.float().view(-1,1),
                                     yFull.float().view(-1,1),
                                     rFull.float().view(-1,1)],axis=1)).to(device)
        ...
        dq5 = 2 * length.view(-1,1) * dt.view(-1,1) * self.dq5_Layer(torch.cat([a1.float().view(-1,1),
                                                               qlast.float().view(-1,1),
                                                               amid.float().view(-1,1),
                                                               a2.float().view(-1,1)],axis=1)).to(device)
        dq6 = (dt.view(-1,1) * 2.5 * v.view(-1,1) / length.view(-1,1)) * self.dq6_Layer(torch.cat([qlast.float().view(-1,1),dt.float().view(-1,1)],axis=1)).to(device)
        q = self.q_Layer(torch.cat([qlast.float().view(-1,1),
                                    ...
                                    dq4.float().view(-1,1),
                                    dq5.float().view(-1,1),
                                    dq6.float().view(-1,1)],axis=1)).to(device)#(qlast.view(-1,1)-dq2+dq3+dq4+dq5+dq6)/denom
        return q
